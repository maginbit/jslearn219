import { fromEvent, timer, of,interval, concat, merge, combineLatest, forkJoin, from, Subject, pipe } from "rxjs";
...
(() =>{
    ...
    // ========================================
        ...
        const conbinacion$ =combineLatest(letras$, numeros$).pipe(
            // map(([a,b])=>a+b)
            map(resp => resp[0])
        );

        conbinacion$.subscribe(console.log)
